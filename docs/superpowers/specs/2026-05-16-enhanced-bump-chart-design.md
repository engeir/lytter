...
The current "Monthly Top 3 Ranking Evolution" plot on the yearly stats page is noisy and
hard to read. Key issues:

- Too many items (15+) creates spaghetti lines
- Rank encodes no magnitude — being #1 with 10 plays looks identical to #1 with 100
  plays
...
Change from "items that appeared in any month's top N" to **top 8 items by total yearly
plays**:
...
**Marker sizing:** `size = BASE_SIZE + plays * SCALE_FACTOR`. BASE_SIZE ~6px.
SCALE_FACTOR chosen so the largest marker in a given chart is ~18px. Computed per-render
from the max play count across all months/items in that tab.

**Line gaps:** When an item has 0 plays in a month, its data point is `null`. Plotly
with `connectgaps: false` renders this as a visible break — one-month wonders appear as
isolated dots surrounded by gaps.

**Annotations:** Right-side item name labels placed at each item's December (or last
non-null month) data point. Font color matches line color. If two items end at the same
rank, offset the annotations by ±0.3 rank units to prevent overlap.
...
preserves accurate ranking context (an item shown at rank #3 is truly #3 among all
tracked items, not just #3 out of 8).
...
- Update card title from "Monthly Top 3 Ranking Evolution" to "Monthly Ranking
  Evolution"
